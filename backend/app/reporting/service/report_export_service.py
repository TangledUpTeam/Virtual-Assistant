...
class ReportExportService:
    ...
    def export_performance_pdf(
        ...
        """
        # DB에서 실적보고서 조회
        performance_report = PerformanceReportRepository.get_by_owner_and_period(
            db, owner, period_start, period_end
        )
        
        if not performance_report:
            raise ValueError(f"실적보고서를 찾을 수 없습니다: {owner} - {period_start}~{period_end}")
        
        # CanonicalReport로 변환
        report_json = performance_report.report_json
        report = CanonicalReport(**report_json)
