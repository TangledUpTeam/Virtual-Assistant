...
def convert_performance_to_canonical(raw: PerformanceReportSchema) -> CanonicalReport:
    """
    실적 보고서 Raw → Canonical 변환
    
    Args:
        raw: PerformanceReportSchema 객체
    ...
    calendar = CalendarData(
        상반기=HalfYear(
            분기1=QuarterMonths(
                월1=raw.달력.상반기.분기1.월1,
                월2=raw.달력.상반기.분기1.월2,
                월3=raw.달력.상반기.분기1.월3
            ),
            분기2=QuarterMonths(
                월1=raw.달력.상반기.분기2.월1,
                월2=raw.달력.상반기.분기2.월2,
                월3=raw.달력.상반기.분기2.월3
            )
        ),
        하반기=HalfYear(
            분기1=QuarterMonths(
                월1=raw.달력.하반기.분기1.월1,
                월2=raw.달력.하반기.분기1.월2,
                월3=raw.달력.하반기.분기1.월3
            ),
            분기2=QuarterMonths(
                월1=raw.달력.하반기.분기2.월1,
                월2=raw.달력.하반기.분기2.월2,
                월3=raw.달력.하반기.분기2.월3
            )
        )
    )
    ...
    issues = []
    if raw.이슈_및_계획:
        if isinstance(raw.이슈_및_계획, list):
            issues = raw.이슈_및_계획
        else:
            issues = [raw.이슈_및_계획] if raw.이슈_및_계획 else []
    
    # notes (비고)
    notes = raw.비고 or ""
    
    # CanonicalPerformance 생성
    canonical_performance = CanonicalPerformance(
        header=header,
        kpis=kpis,
        calendar=calendar,
        issues=issues,
