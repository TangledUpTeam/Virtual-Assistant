...
class CanonicalPerformance(BaseModel):
    """실적 보고서 Canonical 구조"""
    header: Dict[str, str] = Field(..., description="상단 정보 (작성일자, 성명)")
    kpis: Dict[str, Any] = Field(default_factory=dict, description="주요 지표")
    calendar: CalendarData = Field(..., description="달력 데이터")
    issues: List[str] = Field(default_factory=list, description="이슈 및 계획")
...
class CanonicalReport(BaseModel):
    ...
    performance: Optional[CanonicalPerformance] = Field(default=None, description="실적 보고서 데이터")
