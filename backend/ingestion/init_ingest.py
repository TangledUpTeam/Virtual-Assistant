...
from ingestion.ingest_reports import ingest_reports
...
def main():
    ...
    for chunks_path in report_chunk_files:
        if Path(chunks_path).exists():
            print(f"📄 보고서 청크 로드 중: {chunks_path}")
            chunks = load_chunks_from_json(chunks_path)
            all_reports_chunks.extend(chunks)
            print(f"✅ 로드 완료: {len(chunks)}개 청크")
        else:
            print(f"⚠️  파일을 찾을 수 없습니다: {chunks_path}")
    
    print(f"\n📊 총 보고서 청크: {len(all_reports_chunks)}개")
    print()
    
    if all_reports_chunks:
        result = ingest_reports(
            chunks=all_reports_chunks,
            api_key=api_key,
            batch_size=100
        )
        
        if result["success"]:
            print("✅ 보고서 Ingestion 성공")
        else:
            print(f"❌ 보고서 Ingestion 실패: {result.get('message', 'Unknown error')}")
    else:
        print("⚠️  업로드할 보고서 청크가 없습니다.")
