...
주요 변경:
  ...
  - 페르소나 캐싱 시스템 (24시간 유효)
  - 백그라운드 스레드로 비동기 페르소나 생성 (초기 로딩 시간 단축)
  - 병렬 검색 처리 (asyncio.gather로 3개 쿼리 동시 실행)
  - 중복 제거 최적화 (set 기반 ID 중복 체크)
  - 검색 쿼리 최적화 (6개 → 3개로 축소, 상위 5개 청크만 사용)
  - 기본 페르소나로 빠른 시작 + 백그라운드에서 RAG 페르소나 생성
사용 기술:
  - 비동기 프로그래밍 (asyncio, AsyncOpenAI)
  - 스레드 기반 백그라운드 작업 (Thread, daemon=True)
  - JSON 기반 캐싱 (파일 시스템)
  - 병렬 API 호출 (asyncio.gather)
시간복잡도 최적화:
  - 검색 쿼리 수 축소: O(6*q*n) → O(3*q*n) (50% 감소)
  - 병렬 검색: O(3*q*n) → O(q*n) (3배 속도 향상)
  - 중복 제거: O(n²) → O(n) (set 사용)
  - 캐싱: O(p + q*n + m) → O(1) (캐시 히트 시)
"""
