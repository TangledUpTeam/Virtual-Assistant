...
[should_search_vector_db = True]
  ...
  ├─→ 최고 유사도 계산
  └─→ Threshold 분기 (0.7)
      ├─→ [유사도 ≥ 0.7 AND RAG 강제 아님] → LLM 단독 답변
      └─→ [유사도 < 0.7 OR RAG 강제] → RAG 기반 답변
          ├─→ RAG 답변 생성
...
[should_search_vector_db = True]
  ...
  └─→ Threshold 분기 (0.7)
      ...
      │     └─→ LLM 단독 답변 생성
      │           └─→ response_generator._generate_llm_only_response
      └─→ [유사도 < 0.7 OR RAG 강제]
            ├─→ RAG 기반 답변 생성
            │     └─→ response_generator.generate_response_with_persona
...
#### ResponseGenerator._generate_llm_only_response (197-363줄)
...
#### ResponseGenerator.generate_response_with_persona (366-610줄)
