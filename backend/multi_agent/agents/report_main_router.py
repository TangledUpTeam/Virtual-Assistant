...
- chunk_type="summary"의 청크만 분석하여 **데이터 기반**으로 작성
- 반복적으로 나타나는 업무 주제/패턴의 **빈도수**를 기반으로 우선순위 결정
- 정확히 **3개의 목표**만 생성
- 각 목표는 실제 데이터에서 발견된 패턴을 반영해야 하며, 추상적 표현 금지
- **반드시 동사형으로 끝나야 함** (예: "~구성", "~제안", "~진행", "~분석", "~정리")
- "~합니다", "~한다", "~하도록 한다" 같은 종결형 사용 금지

예시 (올바른 형식):
- "보장이슈 5건을 기준으로 보장안 5건 구성"
- "고객 2곳에 보험 리모델링 제안 진행"
- "상담 자료 10건 정리 및 분석"
...
- chunk_type="detail"의 청크를 date 기준으로 분류
- **반드시 해당 주의 월요일~금요일 5일 모두 포함** ("2025-11-03", "2025-11-04", "2025-11-05", "2025-11-06", "2025-11-07")
- 날짜는 YYYY-MM-DD 형식으로 통일
- **각 날짜별로 정확히 3개의 업무를 추출** (더 많거나 적게 하지 마세요)
- 각 업무는 원본 detail 청크의 의미적 내용을 최대한 보존해야 합니다
- 관련된 액션을 하나의 대표적인 업무로 결합하는 것을 선호하세요

**고객명 처리 규칙 (매우 중요 - 절대 위반 금지):**
- **고객명이 포함된 업무 문장을 절대 삭제하거나 버리지 마세요**
- **고객명만 제거하고 나머지 업무 문장은 그대로 유지하세요**
- 예: "박새린 고객 상담 자료 정리 및 니즈 분석" → "상담 자료 정리 및 니즈 분석" (업무 유지, 고객명만 제거)
- 예: "포트폴리오 진단 고객(김라해) 리포트 작성" → "포트폴리오 진단 고객 리포트 작성" (업무 유지, 고객명만 제거)
- 예: "신규 리드 상담(노지유)" → "신규 리드 상담" (업무 유지, 고객명만 제거)
- 고객명이 있어도 해당 업무를 건너뛰거나 삭제하지 마세요
...
1) 상담 자료 정리 및 니즈 분석
2) 포트폴리오 진단 리포트 작성
3) 신규 리드 상담 및 절세 플랜 점검 1건

**허용된 변환:**
- 고객명 제거하되 의미는 보존
- 의미적으로 관련된 여러 detail 항목을 하나의 더 풍부한 업무로 결합
- 동사와 도메인 용어는 그대로 유지
- 시간 범위 제거
- 의미 없는 필러 액션(예: "점심") 제거 (단, 보고서에 의미가 있는 경우는 유지)

**금지된 행동:**
- 원본 데이터에 그대로 나타나지 않는 템플릿 업무(예: "상담 자료 정리 / 보장안 구성 / 리포트 작성")를 출력하지 마세요
- 여러 날짜에 걸쳐 같은 3개 업무를 반복하지 마세요
- detail 청크에 존재하지 않는 업무를 환상하지 마세요
- 업무를 일반적인 짧은 구문으로 단순화하지 마세요

**반복 방지 규칙 (매우 중요):**
- **각 날짜의 업무는 반드시 서로 달라야 합니다**
- 같은 업무 목록을 모든 요일에 재사용하지 마세요
- 각 날짜의 업무는 해당 날짜의 실제 detail 청크를 반영해야 합니다
- 각 날짜별로 고유한 업무 패턴을 반영하세요

**시간 정보 제거:**
- 시간 정보(예: [09:00-10:00], [10:00-11:00])는 **완전히 제거**

**필수 출력 스타일:**
- **모든 요일별 업무는 행동 지향적 동사형으로 끝나야 합니다**: "~정리", "~작성", "~구성", "~분석", "~점검"
- "~합니다" 또는 "~한다"를 절대 사용하지 마세요

출력 형식:
{
  "2025-11-03": ["업무1 (동사형)", "업무2 (동사형)", "업무3 (동사형)"],
  "2025-11-04": ["업무1 (동사형)", "업무2 (동사형)", "업무3 (동사형)"],
  "2025-11-05": ["업무1 (동사형)", "업무2 (동사형)", "업무3 (동사형)"],
  "2025-11-06": ["업무1 (동사형)", "업무2 (동사형)", "업무3 (동사형)"],
  "2025-11-07": ["업무1 (동사형)", "업무2 (동사형)", "업무3 (동사형)"]
}

### 3. 주간 중요 업무 (weekly_highlights)
- chunk_type="pending"의 모든 청크를 분석
- 다음 키워드와 관련된 항목만 선별: "미종결", "지연", "콜백", "진행중", "미완료", "처리 못함"
- **빈도수 기반으로 Top 3 반복 패턴만 추출**
- **중요 규칙**:
  * **고객 이름은 완전히 제거** (예: "문은가", "박이린" 등)
  * 동일한 업무 패턴이 여러 고객에 대해 반복되면 하나로 통합
  * 예: "문은가 고객 추가 자료 대기" → "추가 자료 대기"
  * 예: "박이린 고객 상담 자료 정리 필요" → "상담 자료 정리"
  * 예: "김태은 고객 보장안 재확인 필요" → "보장안 재확인"
- **각 항목은 반드시 동사형으로 끝나야 함** (예: "자료 대기", "보장안 재확인", "상담 자료 정리")
- "~합니다", "~한다", "~필요" 같은 종결형 사용 금지
- 정확히 **3개만** 반환

### 4. 메모/비고 (notes)
- chunk_type="plan"의 청크에서 메모 성격 문장을 추출
- **"메모:" 접두어를 절대 사용하지 마세요.** 접두어 없이 내용만 추출
- 고객 반응, 컴플레인, 콜백 요청 사항을 중심으로 기록
- 여러 날짜의 메모가 있으면 **"\n"으로 구분하여 하나의 문자열로 병합**
- 예: "고객 A 콜백 요청\n고객 B 추가 자료 요청\n고객 C 계약 조건 재협상 요청"
- 추측하거나 만들어낸 메모는 절대 포함하지 마세요. 원본 데이터에 있는 내용만 사용
...
## 중요 규칙
1. 제공된 검색 결과만을 근거로 작성하세요. 추측은 금지합니다.
2. JSON만 출력하세요. 다른 텍스트는 금지합니다.
3. 날짜는 YYYY-MM-DD 형식만 사용하세요.
4. 모든 문장은 동사형으로 끝나야 합니다 ("~작성", "~정리", "~분석" 등). "~합니다", "~한다" 사용 금지.
5. 요일별 업무는 정확히 3개를 추출하세요. 고객명이 포함된 업무 문장을 절대 삭제하지 마세요. 고객명만 제거하고 업무는 보존하세요.
6. 메모에는 "메모:" 접두어를 사용하지 마세요.
7. 배열이나 문자열이어도 동일한 필드는 모두 반환하세요.
8. 각 날짜의 업무는 서로 달라야 하며, 템플릿처럼 반복되지 않아야 합니다.
"""
